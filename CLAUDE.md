...
- **활성/선택/클릭 상태에서 겉 테두리에 상징색(앰버 등 액센트) 노출 금지.** 선택 상태는 배경 1단(`bg-active`)과 텍스트 톤으로만 구분한다. 카드·행·칩·인풋 전부 해당. *예외: 키보드 `:focus-visible` 링은 접근성 장치라 유지(§DESIGN 7). 마우스 클릭·선택 상태에 액센트 보더를 칠하는 것이 slop이다.* (지시 2026-06-12)
...
- 엔트런스 애니메이션·skeleton shimmer·spinner 금지. 로딩은 평문 한 줄.
...
- 메일 본문·자격증명은 브라우저 번들로 들어오지 않는다. 로컬 데몬(localhost) 경유로만 조회.
