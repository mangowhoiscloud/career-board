...
- **FilterBar**: 26px 칩, 8px 도트 + 라벨 + mono 카운트. hover=bg-hover(일시적), **active=텍스트 톤 600 + 카운트 앰버 — 박스·보더·언더라인 금지** (윤곽선 마크 = slop, CLAUDE.md 참조). 탈락 3종·보류는 세로 디바이더로 그룹 분리. Search = 헤어라인 언더라인 인풋 + ⌘K kbd 힌트(실제 단축키 와이어링).
